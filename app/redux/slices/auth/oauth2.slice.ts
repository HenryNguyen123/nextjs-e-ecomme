...
export const fetchLoginByOauth2 = createAsyncThunk<void, LoginOauthType>(
  ...
  (data) => {
    ...
    let keyUrl = process.env.NEXT_PUBLIC_SERVER_URL || ''
    switch(title) {
        case 'google':
            keyUrl += process.env.NEXT_PUBLIC_LOGIN_GOOGLE_USER_URL
            break;
        case 'facebook':
            keyUrl += process.env.NEXT_PUBLIC_LOGIN_FACEBOOK_USER_URL
            break; 
    }
