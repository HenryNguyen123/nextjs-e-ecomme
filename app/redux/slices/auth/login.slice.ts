...
export const loginAuthentication = createAsyncThunk<DataResponse, LoginData>(
  'auth/login',
  async (data) => {
    const response =  await axios.post<DataResponse>(process.env.NEXT_PUBLIC_LOGIN_USER_URL ?? '', {userName: data.userName, password: data.password, rememberUser: data.rememberUser})
