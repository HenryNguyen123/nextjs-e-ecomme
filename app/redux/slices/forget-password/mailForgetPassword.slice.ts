...
export const sendcheckMailForgotPassword = createAsyncThunk<DataResponse, DataType>(
...
export const sendMailForgotPasswordSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
    .addCase(sendcheckMailForgotPassword.pending, (state) => {
        state.isLoading = true
        state.isError = false
    })
    .addCase(sendcheckMailForgotPassword.fulfilled, (state, action: PayloadAction<DataResponse>) => {
      console.log('payload check mail forget password: ', action.payload)
        state.data = action.payload ? action.payload : null
        state.isLoading = false
        state.isError = false
    })
    .addCase(sendcheckMailForgotPassword.rejected, (state) => {
        state.isLoading = false
        state.isError = true
    })
  },
